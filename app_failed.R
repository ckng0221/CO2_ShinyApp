                      ...
                      div(class="outer",
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        sliderTextInput("plot_date",
                                                        ...
                                                        choices = format(unique(cv_cases$date), "%Y"), #%d %b %y
                                                        selected = format(current_date, "%Y"), #ori: %d %b %y
                                                        grid = FALSE,
...
server = function(input, output, session) {
  ...
  formatted_date = reactive({
    format(as.Date(input$plot_date, format="%Y"), "%Y-%m-%d") #ori: format(as.Date(input$plot_date, format="%d %b %y"), "%Y-%m-%d")
  })
  
  # output$clean_date_reactive <- renderText({
  #   format(as.POSIXct(formatted_date()),"%Y") #ori:     format(as.POSIXct(formatted_date()),"%d %B %Y")
  # })
