...
sars_new_cases_plot = function(sars_aggregated, plot_date) {
  ...
  ggplot(plot_df_new, aes(x = date, y = new)) + 
    ...
    xlim(c(sars_min_date,sars_max_date)) + scale_x_date(date_labels = "%y", limits=c(sars_min_date,sars_max_date)) +
    theme(legend.title = element_blank(), legend.position = "", plot.title = element_text(size=10), 
          plot.margin = margin(5, 5, 5, 5))
}
...
country_cases_cumulative = function(cv_cases, start_point=c("Date", "Week of 100th confirmed case",
                                                            "Week of 10th death"), plot_start_date) {
  if (start_point=="Date") {
...
if (any(grepl("/", cv_cases$date))) { 
  cv_cases$date = format(as.Date(cv_cases$date, format="%d/%m/%Y"),"%y")  #"%d/%m/%Y" #"%Y-%m-%d"
                      ...
                      div(class="outer",
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        sliderTextInput("plot_date",
                                                        ...
                                                        choices = format(unique(cv_cases$date), "%y"), #ori: %d %b %y
                                                        selected = format(current_date, "%y"),
                                                        grid = FALSE,
...
server = function(input, output, session) {
  ...
  formatted_date = reactive({
    format(as.Date(input$plot_date, format="%y"), "%y") #ori : "%d %b %y")
  })
  
  # output$clean_date_reactive <- renderText({
  #   format(as.POSIXct(formatted_date()),"%y")
  # })
  # 
